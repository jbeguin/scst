...
struct corw_handler *corw_handler_create(char *corwfile, char *file, int64_t file_size, int block_size) {
    ...
    corwh->buf = buf;
    corwh->nblocks = (file_size + block_size - 1) / block_size;
    size_t bitmaplen = 0;
    corwh->bitmap = bitmap_open_file(bitmapfn, corwh->nblocks, &bitmaplen, 0, 0);
    corwh->bitmap_fn = bitmapfn;
    corwh->bitmaplen = bitmaplen;
	PRINT_INFO("corwh->bitmaplen %d", (int) corwh->bitmaplen);
	PRINT_INFO("corwh->file_size %d", (int) corwh->file_size);
...
ssize_t load_block(struct corw_handler *h, int fuse_fd, int i) {
    ...
    ssize_t res = read(fuse_fd, buf, h->block_size);
    if (res < 0) {
...
ssize_t corw_handler_load_range(struct corw_handler *h, int fuse_fd, loff_t loff, int length) {
    ...
    TRACE_DBG("start_block %d end_block %d", start_block, end_block);
    for (int i = start_block; i <= end_block; i++) {
        TRACE_DBG("i %d bitmap_test(h->bitmap, i) %d", i, bitmap_test(h->bitmap, i));
        if (!bitmap_test(h->bitmap, i)) {
            res = load_block(h, fuse_fd, i);
            if (res < 0) {
                PRINT_ERROR("block %d fuse_fd %d", i, fuse_fd);
...
ssize_t corw_handler_load_write_range(struct corw_handler *h, int fuse_fd, loff_t loff, int length) {
    ...
    if (!bitmap_test(h->bitmap, start_block)) {
        res = load_block(h, fuse_fd, start_block);
        if (res < 0) {
            PRINT_ERROR("block %d fuse_fd %d", start_block, fuse_fd);
        } else {
            bitmap_on(h->bitmap, start_block);
        }
    }
    if (start_block != end_block) {
        TRACE_DBG("end_block %d bitmap_test(h->bitmap, end_block) %d", end_block, bitmap_test(h->bitmap, end_block));
        if (!bitmap_test(h->bitmap, end_block)) {
            load_block(h, fuse_fd, end_block);
            res = load_block(h, fuse_fd, end_block);
            if (res < 0) {
                PRINT_ERROR("block %d fuse_fd %d", end_block, fuse_fd);
            } else {
