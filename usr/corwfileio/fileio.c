...
int main(int argc, char **argv)
{
	...
	while ((ch = getopt_long(argc, argv, "+b:C:B:e:LtrongluF:I:cp:f:m:d:vsS:P:hDR:Z:M:",
		...
		case 'b':
			...
			break;
		case 'C':
			corw_filename = malloc(strlen(optarg) + 1);
			strcpy(corw_filename, optarg);
			PRINT_INFO("corw_filename %s (%s)", corw_filename, optarg);
			break;
		case 'B':
			size = atoi(optarg);
			if (size > 0)
				corw_block_size = size;
			PRINT_INFO("corw_block_size %x (%s)", corw_block_size, optarg);
			break;
		case 'e':
